...
First, set up the environment based on the server Cargo.toml. It may be more efficient to use versions already downloaded in the local cargo registry. <sidenote>For on-chain program development, use the `solana-program` library; for off-chain clients, use `solana-sdk`. Specifying `@3.0.0` without the `=` prefix resolves to `^3.0.0`, which finds the latest compatible version.</sidenote>
