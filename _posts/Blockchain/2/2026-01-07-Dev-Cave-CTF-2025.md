...
First, the debugger feature must be enabled. Forward the feature appropriately for the version. <sidenote>In VS Code's `launch.json`, the environment variable `VM_DEBUG_PORT` was added but commented out, because the approach was changed to set the environment variable immediately before debugging the targeted transaction.</sidenote>
